...
(: 05/05/2010 		2.0.0 		Sukhpreet Kaur 			Initial version :)

declare namespace xf = "http://soa.o2.co.uk/ManageOrder_2_0/transformations/GetArchiveOrderDetailsRSToGetOrderDetailsRS/";
...
declare function xf:GetArchiveOrderDetailsRSToGetOrderDetailsRS($getArchiveOrderDetailsResponse as element(ns2:getArchiveOrderDetailsResponse),
    ...
    as element(ns0:getOrderDetailsResponse) {
    	...
    	let $getOrderDetailsConfigData := $configXml/getOrderDetails
        return
        <ns0:getOrderDetailsResponse>
            ...
            <ns0:orderHeaderDetails>
                ...
                <ns0:orderDate>{ data($order/ns2:orderDate) }</ns0:orderDate>
                {
                if(data($order/ns2:saleDealer1) = data($configXml/saleDealer1)) then
                ...
                <ns0:sourceOfSale>{ data($configXml/Online) }</ns0:sourceOfSale>
                {
                if(data($order/ns2:status)) then
	                <ns0:orderStatus>{ fn:substring-after(data($order/ns2:status),data($configXml/delimiterColon)) }</ns0:orderStatus>
	            else
	            	<ns0:orderStatus>{ data($configXml/Unknown) }</ns0:orderStatus>
	            }
                <ns0:portalId>{ data($configXml/Unknown) }</ns0:portalId>
                <ns0:customerTier>{ data($configXml/Unknown) }</ns0:customerTier>
                {
                ...
                <ns0:vatTotal>{ data($configXml/ZeroValue) }</ns0:vatTotal>
            </ns0:orderHeaderDetails>
            <ns0:orderDetails>
            {
            ...
            return
                <ns0:package>
                    ...
                    <ns0:item>
                        ...
                        if(data($lineItem/ns2:productCode)) then
                        	...
                        	<ns0:itemId>{ data($configXml/Unknown) }</ns0:itemId>
                        }
                        {
                        if(data($lineItem/ns2:prodDescription)) then
                        	<ns0:itemName>{ data($lineItem/ns2:prodDescription) }</ns0:itemName>
                        else
                        	<ns0:itemName>{ data($configXml/Unknown) }</ns0:itemName>
                        }
                        <ns0:change>{ data($configXml/AddedItem) }</ns0:change>
                        {
                            for $imeiProductType in $getOrderDetailsConfigData/imeiProductType/productType
                            return
                            if(fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon)) = $imeiProductType and data($lineItem/ns2:serialNo)) then
                        ...
                        }
                        {
                            ...
                            return
                            if(fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon)) = $ssnProductType and data($lineItem/ns2:serialNo)) then
                                <ns0:ssn>{ data($lineItem/ns2:serialNo) }</ns0:ssn>
                            else ''
                        }
                        {
							if(((data($getOrderDetailsConfigData/serialNoProductType[productType = fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon))])) or (not(fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon)) = data($getOrderDetailsConfigData/productTypeToCategoryMappings/productTypeToCategoryMapping/productTypes/productType)))) and data($lineItem/ns2:serialNo)) then
								<ns0:serialNo>{ data($lineItem/ns2:serialNo) }</ns0:serialNo>
							else ''
						}
						{
							if(fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon)) = data($getOrderDetailsConfigData/productTypeToCategoryMappings/productTypeToCategoryMapping/productTypes/productType)) then
								...
								where data($productType) = fn:substring-before(data($lineItem/ns2:productType),data($configXml/delimiterColon))
								return
                ...
                <ns0:VATReceiptRequired>{ data($configXml/Yes) }</ns0:VATReceiptRequired>
            </ns0:orderDetails>
            {
            ...
            <ns0:deliveryDetails>
            	...
            	if(data($order/ns2:shipToName)) then
               		...
               		<ns0:name>{ data($configXml/Unknown) }</ns0:name>
               	}
                <ns0:basicAddress>
                    {
                    ...
                    <ns0:country>{ data($configXml/countryNameGeographical) }</ns0:country>
                    {
                    	if(data($order/ns2:shipToPostCode)) then
                    		<ns0:postCode>{ data($order/ns2:shipToPostCode) }</ns0:postCode>
                    	else 
                    		<ns0:postCode>{ data($configXml/Unknown) }</ns0:postCode>
                    }
                </ns0:basicAddress>
            </ns0:deliveryDetails>
        </ns0:getOrderDetailsResponse>
...
xf:GetArchiveOrderDetailsRSToGetOrderDetailsRS($getArchiveOrderDetailsResponse,
    $configXml)